...
public abstract class Entity{
  ...
  /** update the position of this entity using basic physics
    ...
    */
  public final void updatePosition(long time, Field field, int id)throws EntityOutOfBoundsException{
    if( id != this.id ){
  ...
  protected void checkCoordinates(Field field) throws EntityOutOfBoundsException{
    ...
    if(this.getX() >= field.maxX-(this.getSprite().getWidth())  || 
       this.getX() <= field.minX+2 ||
       this.getY() >= field.maxY-(this.getSprite().getHeight()) ||
       this.getY() <= field.minY+2 ){
      throw new EntityOutOfBoundsException("out of bounds");
    }
  ...
  public boolean stop=false; //flag for if player is stopped <================================================ Edited
